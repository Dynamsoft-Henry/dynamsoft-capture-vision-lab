...
public class MrzData implements Serializable {
	...
	static MrzData fromParsedResultItem(@Nullable ParsedResultItem item) {
		if (item == null || item.getParsedFields() == null) {
			return null;
		}
		...
		data.dateOfExpire = (Integer.parseInt(map.get("expiryYear")) + 2000) + "-" + map.get("expiryMonth") + "-" + map.get("expiryDay");
		...
		data.age = (currentDayNumber - birthNumber) / 10000;
		if (data.age >= 100) {
			data.age -= 100;
			birthYear += 100;
		}
